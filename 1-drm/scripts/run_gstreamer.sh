gst-launch-1.0 -e \
    ...
    \
    video_scale_tee. ! queue name=video_transcode_enc_queue ! videoconvert ! "video/x-raw,format=I420" ! x264enc bitrate=2000 byte-stream=false key-int-max=25 bframes=0 aud=false tune=zerolatency pass=cbr ! "video/x-h264,profile=baseline,stream-format=avc" ! tee name=video_transcoded_enc_tee \
    \
